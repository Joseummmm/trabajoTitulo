...
from Metaheuristics.AOA import iterarAOA
...
def solverSCP(id, mh, maxIter, pop, instancia, DS, repairType, param):
    ...
    dirTimes = "./TiemposEjecucion/"
    dirFitness = "./MejoresFitness/"

    # Crear directorio de tiempos si no existe
    for dir in [dirResult, dirTimes, dirFitness]:
        if not os.path.exists(dir):
            os.makedirs(dir)

    # Archivo para almacenar tiempos
    iter_times_file = os.path.join(dirTimes, "iteration_times.csv")
    total_times_file = os.path.join(dirTimes, "total_times.csv")
    fitness_file = os.path.join(
        dirFitness, f'{mh}_{instancia.split(".")[0]}_best_fitness.csv'
    )

    # Crear archivos de tiempos si no existen
    if not os.path.exists(iter_times_file):
        with open(iter_times_file, "w") as f:
            f.write(
                "timestamp,metaheuristic,instance,iteration,iteration_time,best_fitness\n"
            )

    if not os.path.exists(total_times_file):
        with open(total_times_file, "w") as f:
            f.write(
                "timestamp,metaheuristic,instance,total_time,final_fitness,population,max_iterations\n"
            )

    if not os.path.exists(fitness_file):
        with open(fitness_file, "w") as f:
            f.write("execution_id,best_fitness,total_time\n")
    ...
    results = open(
        dirResult + mh + "_" + instancia.split(".")[0] + "_" + str(id) + ".csv", "w"
    )
    ...
    if mh == "GA":
        print(
            "COMIENZA A TRABAJAR LA METAHEURISTICA "
            + mh
            + " / Reparacion: "
            + repairType
        )
    ...
    print(
        ...
        + str(BestFitness)
        + ", mejor iter: "
        + str(fitness[solutionsRanking[0]])
        + ", peor iter: "
        + str(fitness[solutionsRanking[pop - 1]])
        + ", optimo: "
        + str(instance.getOptimum())
        + ", time (s): "
        + str(round(tiempoInicializacion2 - tiempoInicializacion1, 3))
        + ", XPT: "
        + str(XPT)
        + ", XPL: "
        + str(XPL)
    ...
    for iter in range(0, maxIter):
        ...
        if mh == "SCA":
            poblacion = iterarSCA(
                maxIter, iter, instance.getColumns(), poblacion.tolist(), Best.tolist()
            )
        if mh == "GWO":
            poblacion = iterarGWO(
                maxIter,
        ...
        if mh == "MFO":
            poblacion, bestSolutions = iterarMFO(
                maxIter,
                iter,
                instance.getColumns(),
                len(poblacion),
        ...
        if mh == "AOA":
            poblacion = iterarAOA(maxIter, iter, poblacion.tolist(), Best.tolist())

        # Binarizo, calculo de factibilidad de cada individuo y calculo del fitness
        for i in range(poblacion.__len__()):

            if mh != "GA":
                poblacion[i] = b.aplicarBinarizacion(
                    poblacion[i].tolist(), DS[0], DS[1], Best, matrixBin[i].tolist()
                )
        ...
        with open(iter_times_file, "a") as f:
            f.write(
                f"{current_timestamp},{mh},{instancia},{iter+1},{str(round(timeEjecuted,3))},{str(BestFitness)}\n"
            )
    ...
    current_timestamp = int(time.time() * 1000)
    with open(total_times_file, "a") as f:
        f.write(
            f"{current_timestamp},{mh},{instancia},{str(round(tiempoEjecucion,3))},{str(BestFitness)},{pop},{maxIter}\n"
        )

    # Registrar fitness
    with open(fitness_file, "a") as f:
        f.write(f"{id},{str(BestFitness)},{str(round(tiempoEjecucion,3))}\n")

    print("Tiempo de ejecucion (s): " + str(tiempoEjecucion))
    results.close()

    binary = util.convert_into_binary(
        dirResult + mh + "_" + instancia.split(".")[0] + "_" + str(id) + ".csv"
    )
