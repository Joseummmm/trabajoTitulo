...
class SCP:
    ...
    def repairComplex(self, solution):
        ...
        while not feasible:
            ...
            r_no_cubiertas[np.argwhere(aux == 0)] = (
                1  # Vector indica las restricciones no cubiertas
            )
            # print(r_no_cubiertas)
            cnc = np.dot(
                r_no_cubiertas, set
            )  # Cantidad de restricciones no cubiertas que cubre cada columna (de tamaño n)
            # print(cnc)
            trade_off = np.divide(
                costs, cnc
            ...
            idx = np.argmin(
                trade_off
            )  # Selecciono la columna con el trade off mas bajo
            # print(idx)
            solution[idx] = 1  # Asigno 1 a esa columna
            feasible, aux = self.factibilityTest(
                solution
            )  # Verifico si la solucion actualizada es factible
            reparaciones += 1
